...
void bh_dump(bheap_t *h) {
    ...
    printf("Heap: \n");
    for(i = 1; i <= h->n; i++) printf(" %d(%lf)", h->a[i].item, h->a[i].key);
    printf("\n");
    
    for(i = 2; i <= h->n; i++) {
        if(h->a[i].key < h->a[i/2].key) {
            printf("key error at entry %d, value %lf\n", i, h->a[i].key);
            exit(1);
        }
    }
    for(i = 1; i <= h->n; i++) {
